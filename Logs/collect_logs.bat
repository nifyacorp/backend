...
:: Collect and append the logs with broader query
echo Collecting backend runtime logs...
gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=backend" --limit=500 --format="value(textPayload)" >> Logs\backend_runtime_logs.txt
